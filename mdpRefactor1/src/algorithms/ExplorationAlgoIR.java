...
public class ExplorationAlgoIR {
	private final Map exploredMap;
	private final Map realMap;
	private final Robot bot;
	private Calibration cab;
	private List<Cell> obstaclesLoc= new ArrayList<>();
	...
	public ExplorationAlgoIR(Map exploredMap, Map realMap, Robot bot) {
	...
	public void runExploration() {
		...
		//this FOR loop puts all the obstacles in the middle area in a list
		for( int row= 3; row <=MapConstants.MAP_ROWS-4; row++){
        	for( int col=3 ;col <=MapConstants.MAP_COLS-4; col++) {
        		Cell tempcell= exploredMap.getCell(row, col);
        		if(tempcell.getIsObstacle() && (!tempcell.getCheckForImg())) {
        			  obstaclesLoc.add(tempcell);
        		}
        	}
        }
		while(!obstaclesLoc.isEmpty()) {
        	System.out.println("HI THERE");
        	System.out.println(obstaclesLoc);
        	Cell tempCell = obstaclesLoc.remove(0);
        	Cell nearestExp = exploredMap.nearestExplored(tempCell);
        	 FastestPathAlgo4 goToIslandObstacle = new FastestPathAlgo4(exploredMap, bot, realMap);
        	 goToIslandObstacle.runFastestPath(nearestExp.getRow(),nearestExp.getCol());
        	 
        	 exploreIslandObstacle(tempCell,nearestExp);
        	 int obstacleLocPointer=0;
        	 while(!obstaclesLoc.isEmpty() && obstacleLocPointer<obstaclesLoc.size() ) {
        		 System.out.println("Step1");
        		 System.out.println(obstaclesLoc);
        		 System.out.println("POINTER: "+ obstacleLocPointer);
        		 if(obstaclesLoc.get(obstacleLocPointer).getCheckForImg()) {
        			 System.out.println("Step2");
        			 obstaclesLoc.remove(obstacleLocPointer);
        	...
        	System.out.println("exited obstacle loop");
	...
	private void exploreIslandObstacle(Cell nearestUnexplored, Cell nearestExplored) {
		boolean islandBlkFlag= false;
		Cell cellForImage;
		int virtualWallCheckCounter=4;
		
		if (isExploredAndFree(nearestUnexplored.getRow(),nearestUnexplored.getCol()) 
	   		 && neighbourObstacleFree(nearestUnexplored.getRow(),nearestUnexplored.getCol())) {
	   	 cellForImage= nearestUnexplored;
	   	FastestPathAlgo4 goToIslandObst = new FastestPathAlgo4(exploredMap, bot, realMap);
	   	goToIslandObst.runFastestPath(nearestExplored.getRow(),nearestExplored.getCol());
	   	goToIslandObst.runFastestPath(cellForImage.getRow(),cellForImage.getCol());
	   	 
	    }
	    else {
	   	 cellForImage = nearestExplored;
	   }
	    	System.out.println("cell for image"+ cellForImage.getRow()+ "col"+cellForImage.getCol());
	    	
	    	
	    	DIRECTION obstacleDir= hasObsOnBotRight(cellForImage,bot.getRobotPosDir());
	    	Calibration.turnBotDirection(obstacleDir);
	    	bot.move(MOVEMENT.LEFT);

	   	 System.out.println("direction in insland before left turn" + bot.getRobotPosDir());
			 
	    do {
	   	 
	   		 System.out.println("moving around island block");
	   		
	   		 System.out.println("direction in insland" + bot.getRobotPosDir());
	   		 senseAndRepaint();
	   		 
	   		 nextMove();
	   		  System.out.println(" crow"+ bot.getRobotPosRow()+ "col" + bot.getRobotPosCol());
	   		 
	     	
	    }while(!(bot.getRobotPosRow()==cellForImage.getRow() && bot.getRobotPosCol()==cellForImage.getCol()));
	  // }
	   System.out.println("after 1 island explore row"+ bot.getRobotPosRow()+ "col" + bot.getRobotPosCol());
	    System.out.println("exited do while loop");
	}
	
	private void markObstacleAsSeen(int curRow, int curCol, DIRECTION dir) {
		 int r,c;
		    r= curRow;
		    c = curCol;

		    if (dir == DIRECTION.NORTH && isExploredAndObstacle(r, c +2) ) {
		         exploredMap.getCell(r, c +2).setCheckForImg(true);
		       
		    }
		    if (dir == DIRECTION.EAST && isExploredAndObstacle(r-2, c)) {
		         exploredMap.getCell(r-2, c ).setCheckForImg(true);
		       
		    }
		    if (dir == DIRECTION.SOUTH && isExploredAndObstacle(r,c-2)) {
		         exploredMap.getCell(r, c -2).setCheckForImg(true);
		       
		    }
		    if (dir == DIRECTION.WEST && isExploredAndObstacle(r+2,c)) {
		         exploredMap.getCell(r+2, c).setCheckForImg(true);
	...
	//this method detects the direction that the obstacle is located
	  private DIRECTION hasObsOnBotRight(Cell cellForImage, DIRECTION curDir) {
	  	    int curRow= bot.getRobotPosRow();
	  	    int curCol= bot.getRobotPosCol();
	  	    DIRECTION obstacleDIrection= null;;
	  	    if(cellForImage.getRow()-curRow>0) {
	  	    	obstacleDIrection= DIRECTION.NORTH;
	  	    }
	  	    if (cellForImage.getRow()-curRow<0) {
	  	    	obstacleDIrection= DIRECTION.SOUTH;
	  	    }
	  	    if(cellForImage.getCol()-curCol>0) {
	  	    	obstacleDIrection= DIRECTION.EAST;
	  	    }
	  	    else {
	  	    	obstacleDIrection= DIRECTION.WEST;
	  	    }
	...
	private boolean neighbourObstacleFree(int row, int col) {
		for(int i=-1; i<=1;i++) { //for row
			for(int j=-1; j<+1; j++) { //for col
				if (exploredMap.getCell(row+i, col+j).getIsObstacle()) {
					return false;
				}
			}
		}
	...
	public void nextMove() {
		...
		if (exploredMap.isFree(row,col,dirCheck)) {
			...
			bot.move(MOVEMENT.RIGHT);
			 markObstacleAsSeen(bot.getRobotPosRow(),bot.getRobotPosCol(),bot.getRobotPosDir());
